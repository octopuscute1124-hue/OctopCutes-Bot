...
const gRepCode = grab(/function getGuildReputation[\s\S]*?\r?\n\}/, 'getGuildReputation');
...
function buildRecord(bl) {
  ...
  const record = new Function('loadBlacklist', 'saveBlacklist', 'isTyposquatOf', 'OFFICIAL_DOMAINS', 'logAction', [tierCode, gRepCode, getCandCode, recordCode, 'return recordScamCandidate;'].join('\n'))(
    () => bl,
    (d) => actions.push(['save']),
    () => false,
    [],
